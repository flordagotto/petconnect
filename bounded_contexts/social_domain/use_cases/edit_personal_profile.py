...
from bounded_contexts.social_domain.entities import BaseProfile, PersonalProfile
from bounded_contexts.social_domain.services.organization_service import (
    OrganizationService,
)
from bounded_contexts.social_domain.services.profile_service import (
    ProfileService,
    ModifyProfileData,
)
...
class EditPersonalProfileUseCase(BaseSocialUseCase):
    @dataclass
    class Request:
        account_id: str
    ...
    async def execute(
        ...
        profile: PersonalProfile = await self.profile_service.get_personal_profile(
            uow=uow,
            entity_id=base_profile.entity_id,
        )

        await self.profile_service.edit_personal_profile(
            uow=uow,
            profile=profile,
            new_profile_data=request.profile_data,
        )

        updated_profile: PersonalProfile = (
            await self.profile_service.get_personal_profile(
                uow=uow,
                entity_id=base_profile.entity_id,
            )
        )

        return self.profile_view_factory.create_personal_profile_view(
