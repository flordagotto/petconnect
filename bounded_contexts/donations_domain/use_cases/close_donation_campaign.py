...
class CloseDonationCampaignUseCase(BaseDonationCampaignUseCase):
    @dataclass
    class Request:
    ...
    async def execute(
        ...
        money_raised: int = await self.donations_service.get_donation_campaign_amount(
            uow=uow, donation_campaign=donation_campaign
        )
        await self.donations_service.close_campaign(
            uow=uow,
