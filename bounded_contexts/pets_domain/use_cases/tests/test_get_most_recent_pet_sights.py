...
from bounded_contexts.pets_domain.use_cases import GetMostRecentLostPetSightsUseCase
...
class TestGetMostRecentPetSightsUseCase(BaseUseCaseTest, BaseTestingUtils):
    TEST_NO_LIMIT = None
    TEST_OFFSET_ZERO = 0
    TEST_LIMIT = 2
    TEST_OFFSET = 1
    ...
    async def initial_data(self, uow: UnitOfWork) -> None:
        ...
        with freeze_time(
            datetime(year=2023, month=10, day=12, hour=10, minute=46, second=0)
        ):
            pet_sight1_recent = await self.create_pet_sight(
                uow=uow,
        ...
        with freeze_time(
            datetime(year=2023, month=10, day=12, hour=10, minute=46, second=15)
        ):
            await self.create_pet_sight(
                uow=uow,
        ...
        with freeze_time(
            ...
            pet_sight2_recent = await self.create_pet_sight(
                uow=uow,
                pet_id=self.lost_pet_data2.entity_id,
                account_id=self.profile.account_id,
            )

        pet_sights_not_ordered: list[PetSight] = [pet_sight1_recent, pet_sight2_recent]
    ...
    async def setUp(self) -> None:
        ...
        self.use_case: GetMostRecentLostPetSightsUseCase = self.dependencies.resolve(
    ...
    async def test_get_pet_sights_success(self) -> None:
        list_view: PetSightListView = await self.use_case.execute(
            GetMostRecentLostPetSightsUseCase.Request(
    ...
    async def test_get_pet_sights_pagination_with_offset(self) -> None:
        list_view: PetSightListView = await self.use_case.execute(
            GetMostRecentLostPetSightsUseCase.Request(
                limit=None, offset=self.TEST_OFFSET
            )
        )
